...
namespace FeatureDemo.Common {
    ...
    public class SplitPanel : Panel {
        ...
        void ThumbPointerReleased(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e) {
            (sender as FrameworkElement).ReleasePointerCaptures();
            Window.Current.CoreWindow.PointerCursor = arrowCursor;
        }
        void ThumbPointerPressed(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e) {
            (sender as FrameworkElement).CapturePointer(e.Pointer);
            Window.Current.CoreWindow.PointerCursor = resizeCursor;
        }
        void ThumbPointerExited(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e) {
            if ((sender as FrameworkElement).PointerCaptures == null || (sender as FrameworkElement).PointerCaptures.Count == 0)
                Window.Current.CoreWindow.PointerCursor = arrowCursor;
        }
        void ThumbPointerEntered(object sender, Windows.UI.Xaml.Input.PointerRoutedEventArgs e) {
            if ((sender as FrameworkElement).PointerCaptures == null || (sender as FrameworkElement).PointerCaptures.Count == 0)
                Window.Current.CoreWindow.PointerCursor = resizeCursor;
        }
        ...
        protected override Size ArrangeOverride(Size finalSize) {
            ...
            foreach(var child in Children) {
                if(IsLayoutStatic && !(child is Thumb)) {
                    child.Clip = new Windows.UI.Xaml.Media.RectangleGeometry() {
