...
namespace GridDemo {
    public class DemoSparkBarControl : Control {
        public static readonly DependencyProperty ActualValueProperty = DependencyProperty.Register("ActualValue", typeof(double), typeof(DemoSparkBarControl), new PropertyMetadata(0, (d, e) => ((DemoSparkBarControl)d).UpdateContent()));
        public static readonly DependencyProperty TargetValueProperty = DependencyProperty.Register("TargetValue", typeof(double), typeof(DemoSparkBarControl), new PropertyMetadata(0, (d, e) => ((DemoSparkBarControl)d).UpdateContent()));
        ...
        void UpdateTargetValue() {
            if(BorderTargetValue == null)
                return;
            BorderTargetValue.Margin = new Thickness(TargetValue / Scale, 0, 0, 0);
        }
        void UpdateActualValue() {
            if(BorderValue == null)
                return;
            BorderValue.Width = ActualValue / Scale;
        }
    ...
    public class DemoSparkColumnChart  : DemoSparkLineChart {
        ...
        PointCollection CreateColumn(List<double> points) {
            ...
            for(int i = 0; i < points.Count; i++) {
                ...
                pointCollection.Add(new Point((Step * i) + offset, MaxAvailableHeight - points[i]/Scale));
                pointCollection.Add(new Point((Step * i) + Step - offset, MaxAvailableHeight - points[i]/Scale));
    ...
    public class DemoSparkLineChart : ContentControl {
        ...
        protected internal virtual double GetStep(Size availableSize) {
            return availableSize.Width / (Points.Count - 1);
        }
