...
Namespace DevExpress.XtraGauges.Demos
    Partial Public Class SysInfo
        ...
        Private Shared Function GetPerfomanceInfo_CPU(ByVal wmiService As WMIService) As PerfomanceInfo_CPU()
            Dim result(-1) As PerfomanceInfo_CPU
            Try
                ...
                result = New PerfomanceInfo_CPU(collection.Length - 1) {}
                For i As Integer = 0 To collection.Length - 1
                    result(i) = New PerfomanceInfo_CPU(
                        CStr(collection(i).Properties("Name").Value),
                        CSng(CULng(collection(i).Properties("PercentProcessorTime").Value)),
                        CSng(CULng(collection(i).Properties("PercentPrivilegedTime").Value)),
                        CSng(CULng(collection(i).Properties("PercentUserTime").Value)))
                Next i
            Catch
            End Try
            Return result
        ...
        Private Shared Function GetPerfomanceInfo_OS(ByVal wmiService As WMIService) As PerfomanceInfo_OS()
            Dim result(-1) As PerfomanceInfo_OS
            Try
                ...
                result = New PerfomanceInfo_OS(collection.Length - 1) {}
                For i As Integer = 0 To collection.Length - 1
                    result(i) = New PerfomanceInfo_OS(
                        CStr(collection(i).Properties("Name").Value),
                        CInt(Fix(CUInt(collection(i).Properties("Processes").Value))),
                        CInt(Fix(CUInt(collection(i).Properties("Threads").Value))))
                Next i
            Catch
            End Try
            Return result
        ...
        Public Overrides ReadOnly Property ShowExportMenu() As Boolean
            ...
            End Get
        End Property
        Public Overrides ReadOnly Property ExportControl() As DevExpress.XtraGauges.Win.GaugeControl
            Get
        ...
        Public Sub New()
            ...
            Using dlg As Utils.WaitDialogForm = New Utils.WaitDialogForm("Connecting WMI Service...", "Please Wait")
        ...
        Sub OnTimerCallback(ByVal state As Object)
            ...
            End If
        End Sub
        Sub InvokeUpdate(ByVal action As Action)
            If IsHandleCreated And DashboardGauge IsNot Nothing Then
                BeginInvoke(action)
            End If
        End Sub
        ...
        End Sub
        Sub OnUpdateSpeedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles updateSpeed.EditValueChanged
            Dim speed As Integer = 50 + (updateSpeed.Properties.Maximum - updateSpeed.Value) * 50
            If pollingTimer IsNot Nothing Then
                pollingTimer.Change(0, speed)
            End If
        End Sub
