...
Namespace DevExpress.XtraGrid.Demos
    ...
    Public Class Relation
        ...
        Public Property Priority() As Priority
            Get
                Return fTask.Priority
            End Get
            Set(ByVal value As Priority)
                fTask.Priority = value
            End Set
        End Property
    End Class
    Public Class Tasks
        ...
        Public Shared Function GetTasksWithPriority(ByVal collection As TasksWithCategories) As Tasks
            Dim ret As New Tasks()
            Dim rnd As New Random()
            Dim names() As String = {"Create the gauges demo design", "Review the new Layout Form", "Check XAF Wizard's new pages", "Create icons for the Layout Control's context menu", "Create Bizcard Design for the blog post", "Create Booth design for TechEd 2015", "Add grayscale icons to the Image Gallery"}
            For i As Integer = 0 To MaxTasks - 1
                ret.List.Add(New Task(collection, i + 1, My.Resources.Task + (i + 1).ToString(), Date.Today.AddDays(rnd.Next(-3, 5))))
                ret(i).TaskName = names(i)
                ret(i).PercentComplete = rnd.Next(1, 99)
                If i = 1 Then
                    ret(i).Priority = Priority.Medium
                End If
                If i = 2 Then
                    ret(i).Priority = Priority.Medium
                End If
                If i = 6 Then
                    ret(i).Priority = Priority.High
                End If
            Next i
            Return ret
        End Function
    ...
    Public Class TasksWithCategories
        ...
        End Sub
        Public Sub New(ByVal usePriorities As Boolean)
            If usePriorities Then
                fTasks = Tasks.GetTasksWithPriority(Me)
            Else
                fTasks = Tasks.GetTasks(Me)
            End If
            fCategories = Categories.GetCategories(Me)
        ...
        Public Shared Function GetTasksWithCategories() As TasksWithCategories
            ...
            Return ret
        End Function
        Public Shared Function GetTasksWithPriorities() As TasksWithCategories
            Dim ret As New TasksWithCategories(True)
            Dim rnd As New Random()
            For i As Integer = 0 To Tasks.MaxTasks - 1
                Dim cat As Category = ret.fCategories(rnd.Next(ret.fCategories.Count))
                If Not ret.HasCategory(ret.fTasks(i), cat) Then
    ...
    Public Class TasksWithCategoriesDatasource
        ...
        Public Sub New(ByVal grid As GridControl)
            Me.fGrid = grid
            Me.fTasks = TasksWithCategories.GetTasksWithPriorities()
