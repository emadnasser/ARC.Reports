...
Namespace DevExpress.XtraScheduler.Demos
	Public NotInheritable Class CarsDataHelper
		...
		Public NotInheritable Class CarBrand
		...
		Private Sub New()
		End Sub
		Public Shared Sub FillData(ByVal control As SchedulerControl, ByVal storage As SchedulerStorage)
			storage.EnableReminders = False
			Dim [date] As DateTime = GetStartDate()
		...
		Public Shared Sub FillStorageData(ByVal storage As SchedulerStorage, ByVal startDate As DateTime)
			FillResources(storage)
			FillAppointments(storage, startDate)
		End Sub

		Public Shared Sub FillResources(ByVal storage As SchedulerStorage)
			storage.Resources.Mappings.Id = "Id"
			storage.Resources.Mappings.Caption = "Caption"
			storage.Resources.DataSource = CreateCars()
		End Sub

		Public Shared Sub FillAppointments(ByVal storage As SchedulerStorage, ByVal startDate As DateTime)
		...
		Public Shared Function CreateCars() As BindingList(Of Car)
			Dim cars As New BindingList(Of Car)()
		...
		Public Shared Function CreateAppointment(ByVal startDate As DateTime) As BindingList(Of CarScheduling)
			Dim reccurenceFormat As String = "<RecurrenceInfo Start=""{0}"" End=""{1}"" WeekOfMonth=""{2}"" WeekDays=""{3}"" Month=""{4}"" OccurrenceCount=""{5}"" Range=""{6}"" Type=""{7}"" Id=""{8}""/>"
			Dim changedOccurrenceFormat As String = "<RecurrenceInfo Id=""{0}"" Index=""{1}""/>"
			Dim apts As New BindingList(Of CarScheduling)()

'			#Region "Audi"
			Dim apt1 As New CarScheduling()
			apt1.EventType = CInt(Fix(AppointmentType.Normal))
...
'			#Region "Chevrolet"
			...
			apt2.EventType = CInt(Fix(AppointmentType.Normal))
			...
			apt3.EventType = CInt(Fix(AppointmentType.Normal))
			...
			apt4.EventType = CInt(Fix(AppointmentType.Normal))
			...
			apt5.EventType = CInt(Fix(AppointmentType.Pattern))
			...
			apt5.RecurrenceInfo = String.Format(CultureInfo.InvariantCulture, reccurenceFormat, recInfo.Start, recInfo.End, CInt(Fix(recInfo.WeekOfMonth)), CInt(Fix(recInfo.WeekDays)), recInfo.Month, recInfo.OccurrenceCount, CInt(Fix(recInfo.Range)), CInt(Fix(recInfo.Type)), recInfo.Id.ToString())
			apts.Add(apt5)

			Dim apt6 As New CarScheduling()
			apt6.EventType = CInt(Fix(AppointmentType.ChangedOccurrence))
...
'			#Region "MercedesBenz"
			...
			apt7.EventType = CInt(Fix(AppointmentType.Normal))
			...
			apt8.EventType = CInt(Fix(AppointmentType.Normal))
			...
			apt9.EventType = CInt(Fix(AppointmentType.Normal))
			...
			apt10.EventType = CInt(Fix(AppointmentType.Normal))
			...
			apt11.EventType = CInt(Fix(AppointmentType.Pattern))
			...
			apt11.RecurrenceInfo = String.Format(CultureInfo.InvariantCulture, reccurenceFormat, recInfo1.Start, recInfo1.End, CInt(Fix(recInfo1.WeekOfMonth)), CInt(Fix(recInfo1.WeekDays)), recInfo1.Month, recInfo1.OccurrenceCount, CInt(Fix(recInfo1.Range)), CInt(Fix(recInfo1.Type)), recInfo1.Id.ToString())

			apts.Add(apt11)

			Dim apt12 As New CarScheduling()
			apt12.EventType = CInt(Fix(AppointmentType.ChangedOccurrence))
			...
			apt13.EventType = CInt(Fix(AppointmentType.DeletedOccurrence))
			...
			apt14.EventType = CInt(Fix(AppointmentType.ChangedOccurrence))
			...
			apt15.EventType = CInt(Fix(AppointmentType.ChangedOccurrence))
			...
			apt16.EventType = CInt(Fix(AppointmentType.DeletedOccurrence))
...
'			#End Region
		...
		Public Shared Function GetStartDate() As DateTime
			...
			Do While [date].DayOfWeek <> DayOfWeek.Wednesday
				[date] = [date].AddDays(1)
			Loop
			Return [date]
		End Function
