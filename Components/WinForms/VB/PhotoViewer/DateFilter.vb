...
Namespace PhotoViewer
	Partial Public Class DateFilter
        ...
        Public Sub New()
            InitializeComponent()
            AddHandler Me._startDate.SizeChanged, AddressOf OnStartDateSizeChanged
        End Sub
        ...
        Private Sub UpdateStartDateLocation()
            Dim pt As Point = New Point(DisplayRectangle.X + (DisplayRectangle.Width - _startDate.Width) \ 2, DisplayRectangle.Y + (panel1.Top - DisplayRectangle.Y - _startDate.Height) \ 2)
            pt.X = Math.Max(0, pt.X)
            pt.Y = Math.Max(0, pt.Y)
            _startDate.Location = pt
        End Sub
