...
Namespace DevExpress.MVVM.Demos.Common.ViewModel
    ...
    Public MustInherit Class SingleObjectViewModelBase(Of TEntity As Class, TPrimaryKey, TUnitOfWork As IUnitOfWork)
        ...
        End Sub
        <DXImage("Reset")> _
        <Display(Name:="Reset Layout")> _
        Public Sub ResetLayout()
            If LayoutSerializationService IsNot Nothing Then
                LayoutSerializationService.Deserialize(Nothing)
            End If
        End Sub
        <Display(AutoGenerateField:=False)> _
        Public Sub OnLoaded()
        ...
        Protected Overridable Sub OnInitializeInRuntime()
            ...
            Messenger.[Default].Register(Of CloseAllMessage)(Me, Sub(m)
                                                                     If m.ShouldProcess(Me) Then
                                                                         OnClosing(m)
                                                                     End If
                                                                 End Sub)
        End Sub
        ...
        End Sub
        Private Sub UpdateTitle(Optional ByVal nullEntityValue As String = Nothing)
            If Entity Is Nothing Then
                _title = nullEntityValue
            Else
                If IsNew() Then
        ...
        End Function
        Public ReadOnly Property IsPrimaryKeyReadOnly As Boolean
            Get
                Return Not IsNew()
            End Get
        End Property
        ...
        End Sub
        Protected Overridable Sub OnLookupCollectionsUpdated()
        End Sub
        ...
        Private Function AppendForeignKeyPredicate(Of TDetailEntity As Class, TDetailProjection As Class, TForeignKey)(ByVal foreignKeyExpression As Expression(Of Func(Of TDetailEntity, TForeignKey)), ByVal projection As Func(Of IRepositoryQuery(Of TDetailEntity), IQueryable(Of TDetailProjection))) As Func(Of IRepositoryQuery(Of TDetailEntity), IQueryable(Of TDetailProjection))
            Dim predicate = ExpressionHelper.GetKeyEqualsExpression(Of TDetailEntity, TDetailEntity, TForeignKey)(foreignKeyExpression, CType(CType(PrimaryKey, Object), TForeignKey))
            Return ReadOnlyRepositoryExtensions.AppendToProjection(Of TDetailEntity, TDetailProjection)(predicate, projection)
        End Function
        ...
        Private ReadOnly Property CanSerialize As Boolean Implements ISupportLogicalLayout.CanSerialize
            ...
            End Get
        End Property
        Private Function SaveState() As SingleObjectViewModelState Implements ISupportLogicalLayout(Of SingleObjectViewModelState).SaveState
            Return New SingleObjectViewModelState() With {.Key = ExpressionHelper.GetKeyPropertyValues(PrimaryKey), .Title = GetTitle(False)}
        End Function
        Private Sub RestoreState(ByVal state As SingleObjectViewModelState) Implements ISupportLogicalLayout(Of SingleObjectViewModelState).RestoreState
            Dim key = If(ExpressionHelper.IsTuple(Of TPrimaryKey)(), ExpressionHelper.MakeTuple(Of TPrimaryKey)(state.Key), CType(state.Key.First(), TPrimaryKey))
            LoadEntityByKey(key)
            If Entity Is Nothing Then
        ...
        Private ReadOnly Property LookupViewModels As IEnumerable(Of Object) Implements ISupportLogicalLayout.LookupViewModels
            Get
                Return _lookUpViewModels.Values
            End Get
        End Property
    End Class
End Namespace
