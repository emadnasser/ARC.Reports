...
Namespace DevExpress.MVVM.Demos.Common.ViewModel
    ...
    Public MustInherit Class DocumentsViewModel(Of TModule As ModuleDescription(Of TModule), TUnitOfWork As IUnitOfWork)
        ...
        Private Sub DestroyOrphanedDocuments()
            Dim orphans = Me.GetOrphanedDocuments().Except(Me.GetImmediateChildren())
        ...
        Public Overridable Sub OnClosing(ByVal cancelEventArgs As CancelEventArgs)
            If GroupedDocumentManagerService IsNot Nothing AndAlso GroupedDocumentManagerService.Groups.Count() > 1 Then
                Dim activeGroup = GroupedDocumentManagerService.ActiveGroup
                Dim message = New CloseAllMessage(cancelEventArgs, Function(vm)
                                                                       Dim activeVMs = activeGroup.Documents.[Select](Function(d) d.Content)
            ...
            If LayoutSerializationService IsNot Nothing Then
                PersistentLayoutHelper.PersistentViewsLayout(_ViewLayoutName) = LayoutSerializationService.Serialize()
            End If
            Messenger.[Default].Send(New CloseAllMessage(cancelEventArgs, Function(vm) True))
        ...
        Public Overridable Sub OnLoaded(ByVal [module] As TModule)
            ...
            If Not RestoreLogicalLayout() Then
                Show([module])
            End If
            PersistentLayoutHelper.TryDeserializeLayout(LayoutSerializationService, _ViewLayoutName)
        End Sub
        ...
        Private Sub OnActiveDocumentChanged(ByVal sender As Object, ByVal e As ActiveDocumentChangedEventArgs)
            ...
            End If
        End Sub
        Protected ReadOnly Property GroupedDocumentManagerService As IGroupedDocumentManagerService
            Get
                Return Me.GetService(Of IGroupedDocumentManagerService)()
            End Get
        End Property
        ...
        Protected ReadOnly Property LayoutSerializationService As ILayoutSerializationService
            Get
                Return Me.GetService(Of ILayoutSerializationService)("RootLayoutSerializationService")
            End Get
        End Property
        ...
        Public Sub SaveLogicalLayout()
            PersistentLayoutHelper.PersistentLogicalLayout = Me.SerializeDocumentManagerService()
        End Sub
        Public Function RestoreLogicalLayout() As Boolean
            If String.IsNullOrEmpty(PersistentLayoutHelper.PersistentLogicalLayout) Then
                Return False
            End If
            Me.RestoreDocumentManagerService(PersistentLayoutHelper.PersistentLogicalLayout)
            Return True
        End Function
