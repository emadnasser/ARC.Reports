...
namespace DevExpress.VideoRent.Win.ModulesKPI {
    public partial class CustomersKPI : ChartExportTutorialControl {
        ...
        void DiscountKPI_Load(object sender, EventArgs e) {
            ...
            discountChart.RegisterSummaryFunction("COUNTWITHZEROS", "COUNTWITHZEROS", 1, new SummaryFunctionArgumentDescription[0], CalcProductValue);
        ...
        SeriesPoint[] CalcProductValue(Series series, object argument, string[] functionArguments, DataSourceValues[] values, object[] colors) {
            return dataSourceHelper.GetSeriesPoints(series.Name, (DateTime)argument, values.Length);
        }
        void BindSeries(PivotGridControl pivotGrid, CustomerDiscountLevel level) {
            ...
            series.SummaryFunction = "COUNTWITHZEROS()";
        }
        ...
        void SetColors(Series series, DrawOptions drawOptions, int colorIndex) {
            PaletteEntry palleteEntry = GetPaletteEntry(colorIndex);
        ...
        void discountChart_CustomDrawSeries(object sender, CustomDrawSeriesEventArgs e) {
            int index = GetSeriesIndex(e.Series.Name);
            if (index >= 0)
                SetColors(e, index);
        }
        void discountChart_CustomDrawCrosshair(object sender, CustomDrawCrosshairEventArgs e) {
            foreach (CrosshairElementGroup elementGroup in e.CrosshairElementGroups) {
                foreach (CrosshairElement elemnt in elementGroup.CrosshairElements) {
                    int index = GetSeriesIndex(elemnt.Series.Name);
                    if (index >= 0) {
                        Color color = GetPaletteEntry(index).Color;
                        elemnt.LabelElement.MarkerColor = color;
                    }
                }
            }
        }
        ...
        PaletteEntry GetPaletteEntry(int colorIndex) {
            return discountChart.GetPaletteEntries(5)[colorIndex];
        }
    ...
    public class DataSourceHelper {
        Dictionary<DateTime, IList<double>> seriesValues = new Dictionary<DateTime, IList<double>>();
        IList<string> seriesNames = new List<string>();

        public SeriesPoint[] GetSeriesPoints(string name, DateTime dateTime, double value) {
            if (!seriesValues.ContainsKey(dateTime))
                AddNewArgument(dateTime);
            int seriesIndex = seriesNames.IndexOf(name);
            if (seriesIndex == -1) {
                AddNewSeries(name);
                seriesIndex = seriesNames.Count - 1;
            }
            if (seriesValues[dateTime][seriesIndex] != value)
                seriesValues[dateTime][seriesIndex] = value;

            return GetPoints(seriesIndex);
        }
        void AddNewArgument(DateTime dateTime) {
            seriesValues.Add(dateTime, new List<double>());
            foreach (string seriesName in seriesNames)
                seriesValues[dateTime].Add(0);
        }
        void AddNewSeries(string name) {
            seriesNames.Add(name);
            foreach (IList<double> values in seriesValues.Values)
                values.Add(0);
        }
        SeriesPoint[] GetPoints(int seriesIndex) {
            List<SeriesPoint> points = new List<SeriesPoint>(seriesValues.Count);
            foreach (KeyValuePair<DateTime, IList<double>> values in seriesValues)
                points.Add(new SeriesPoint(values.Key, values.Value[seriesIndex]));
