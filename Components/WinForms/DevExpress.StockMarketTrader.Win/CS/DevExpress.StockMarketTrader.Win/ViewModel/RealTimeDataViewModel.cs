...
namespace DevExpress.StockMarketTrader.ViewModel {
    public class RealTimeDataViewModel : ViewModelBase {
        ...
        int volumeCount = 15;
        int countOfWeek = 1;
        int candlesCount = 72, currentDate = 0, timeInterval = 1, tickCount = 0, offlineTimeInterval = 500, totalTicks = 5;
        ...
        public RealTimeDataViewModel(Timer timer) {
            ...
            ChangeTimerInterval(timeInterval);
        ...
        void OnUpdateFailed(object sender, RealTimeDataEventArgs e) {
            if (TimerCore == null || TimerCore.Interval != GetTimeInterval())
                ChangeTimerInterval(GetTimeInterval());
            StartTimer();
        ...
        void ChangeTimerInterval(int time) {
            InvokeSync(new Action(() => { TimerCore.Interval = time; }));
        }
        void StartTimer() {
        ...
        void GetStockChartBindingDataAsync() {
            ...
            model.BeginGetCompanyMultipleDataFromPeriod(currentDate, candlesCount, totalTicks, selectedCompanyName);
            currentDate += totalTicks * (candlesCount);
        }
        ...
        void UpdateWatchListBindingDataAsync() {
            ...
            if(currentDate >= 0 && currentDate < dates.Count)
                ...
                model.BeginGetStockDataByDate(dates.FirstOrDefault());
        }
        ...
        void UpdateData() {
            lock (sem) {
                if (dates == null) return;
                StopTimer();
                DateTime date = dates[currentDate];
                switch (totalTicks) {
                    case 5:
        ...
        void UpdateVolumeDynamicsAsync() {
            ...
            if (companies != null) model.BeginGetStockDataFromDateByCompanyList(dates[currentDate], companies.ToArray());
        }
        ...
        void UpdateStockChartBindingDataAsync() {
            ...
            model.BeginGetCompanyStockData(dates[currentDate], selectedCompanyName);
        }
        ...
        void UpdateStockChart(StockData newChartData) {
            ...
            //int chartCount = stockChartBindingData.Count;
            //DateTime old = stockChartBindingData[chartCount - 1].Date;
            switch (totalTicks) {
                case 5:
        ...
        void OnGetDataCompleted(object sender, RealTimeDataEventArgs e) {
            OnPropertyChanged("NetworkState");
            canEndUpdate = e.CanNewUpdate;
            Delegate method = GetDefferedDelegate(e.Key);
            if (method != null && Application.OpenForms.Count > 0) {
                for (int i = 0; i < Application.OpenForms.Count; i++) {
                    if (Application.OpenForms[i] is StockMarketView) {
                        Application.OpenForms[i].Invoke(method, new object[] { e.Data });
                        break;
                    }
                }
            }
        }
        void AddDefferedDelegate(string key, Delegate method) {
            if (!IsSuspendUpdating) {
        ...
        int GetTimeInterval() {
            return model.IsOnline ? timeInterval : offlineTimeInterval;
        }
