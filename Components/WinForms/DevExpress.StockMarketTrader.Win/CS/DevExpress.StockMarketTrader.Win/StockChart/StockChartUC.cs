...
namespace DevExpress.StockMarketTrader {
    public partial class StockChartUC : UserControl {
        ...
        Image LoadImageFromResource(string imageName) {
            return Image.FromStream(Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format(@"DevExpress.StockMarketTrader.Images.{0}.png", imageName)));
        }
        ...
        void StockChartUC_Load(object sender, EventArgs e) {
            if (StockMarketView.defaultViewModel == null)
                return;
            stockBarCheckItem.Glyph = LoadImageFromResource("Stock");
            candleStickBarCheckItem.Glyph = LoadImageFromResource("Candles");
            volumesBarCheckItem.Glyph = LoadImageFromResource("Bars");
            barCheckItem3.Glyph = LoadImageFromResource("6m");
            barCheckItem4.Glyph = LoadImageFromResource("1y");
            barCheckItem5.Glyph = LoadImageFromResource("1-5y");
            barCheckItem6.Glyph = LoadImageFromResource("2y");
            barCheckItem7.Glyph = LoadImageFromResource("4y");
            barCheckItemRemoveIndicator.Glyph = LoadImageFromResource("Remove");
            barCheckItemTrendLine.Glyph = LoadImageFromResource("TrendLine");
            barCheckItemFibonacciArcs.Glyph = LoadImageFromResource("FibonacciArcs");
            barCheckItemFibonacciFans.Glyph = LoadImageFromResource("FibonacciFans");
            barCheckItemFibonacciRetracement.Glyph = LoadImageFromResource("FibonacciRetracement");
            OnPeriodChanged(barCheckItem5, null);
        }
        void OnShowVolumeChartChanged(object sender, ItemClickEventArgs e) {
            Diagram.Panes[0].Visible = volumesBarCheckItem.Checked;
        }
