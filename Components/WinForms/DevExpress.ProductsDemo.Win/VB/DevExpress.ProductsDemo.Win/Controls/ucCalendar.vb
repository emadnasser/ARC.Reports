...
Namespace DevExpress.ProductsDemo.Win.Controls
	Partial Public Class ucCalendar
		...
		Public Sub New()
			...
			End If
			InitializeComponent()
			AddHandler Me.treeResources.LayoutUpdated, AddressOf treeResources_LayoutUpdated
			AddHandler Disposed, AddressOf ucCalendar_Disposed
		End Sub

		Private Sub treeResources_LayoutUpdated(ByVal sender As Object, ByVal e As EventArgs)
		...
		Private Function GetParentNodeState(ByVal nodes As TreeListNodes) As CheckState
			Dim state As CheckState = nodes(0).CheckState
			For Each node As TreeListNode In nodes
				If node.CheckState <> state Then
		...
		Protected Overrides Sub OnLoad(ByVal e As EventArgs)
			...
			End If
		End Sub
		Private Sub ucCalendar_Disposed(ByVal sender As Object, ByVal e As EventArgs)
			If Not DesignTimeTools.IsDesignMode Then
				If LookAndFeel IsNot Nothing AndAlso LookAndFeel.ActiveLookAndFeel IsNot Nothing Then
