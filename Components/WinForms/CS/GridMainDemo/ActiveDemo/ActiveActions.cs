...
namespace DevExpress.ActiveDemos {
	...
	public class ActiveActions : object, IMessageFilter, IDisposable {
		...
		const int DefaultMouseMoveDelay = 20;
		...
		public void SendKey(Control control, char key) {
			if(Canceled) return;
			SendKeyCore(IsControlExists(control) ? control: ActiveControl, key);
			SendKeys.Flush();
		}
		public void SendString(Control control, string keys) {
			if(Canceled) return;
            SendStringCore(IsControlExists(control) ? control : ActiveControl, keys);
			SendKeys.Flush();
		}
        bool IsControlExists(Control control) {
            return !(control == null || control.IsDisposed);
        }
		public void MoveMousePointTo(Control control, Point pt) {
            Point screenPoint = IsControlExists(control) ? control.PointToScreen(pt) : pt;
			MoveMousePointTo(screenPoint);
		}
		public void MoveMousePointTo(Point pt) {
			if(Canceled) return;
			int count = 0;
            int DeltaX = 65536 / Screen.GetBounds(pt).Width * 2;
            int DeltaY = 65536 / Screen.GetBounds(pt).Height * 2;
            pt = ConvertPointToAbsolute(pt);
            while(!IsAbsolutePointEquals(ConvertPointToAbsolute(Cursor.Position), pt, DeltaX, DeltaY)) {
				if(Canceled) return;
                Point cpt = ConvertPointToAbsolute(Cursor.Position);
				int dX = cpt.X - pt.X, dY = cpt.Y - pt.Y;
                if(cpt.X == pt.X) {
					dX = 0;
                } else {
                    int deltaX = Math.Abs(cpt.X - pt.X) > DeltaX ? DeltaX : Math.Abs(cpt.X - pt.X);
                    dX = cpt.X < pt.X ? deltaX : -deltaX;
				}
                if(cpt.Y == pt.Y) {
					dY = 0;
                } else {
                    int deltaY = Math.Abs(cpt.Y - pt.Y) > DeltaY ? DeltaY : Math.Abs(cpt.Y - pt.Y);
                    dY = cpt.Y < pt.Y ? deltaY : -deltaY;
				}
				cpt.X += dX;
				cpt.Y += dY;
                ActiveActionsImports.mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, cpt.X, cpt.Y, 0, IntPtr.Zero);
                //System.Diagnostics.Debug.WriteLine(string.Format("{0}, {1}, {2}", cpt, pt, dX));
				DoEvents();
        ...
        Point ConvertPointToAbsolute(Point pt) {
            pt.X = Convert.ToInt32((65536.0 * pt.X / Screen.GetBounds(pt).Width) + (65536.0 / Screen.GetBounds(pt).Width / 2));
            pt.Y = Convert.ToInt32((65536.0 * pt.Y / Screen.GetBounds(pt).Height) + (65536.0 / Screen.GetBounds(pt).Height / 2));
            return pt;
        }
        bool IsAbsolutePointEquals(Point pt1, Point pt2, int deltaX, int deltaY) {
            return Math.Abs(pt1.X - pt2.X) <= deltaX && Math.Abs(pt1.Y - pt2.Y) <= deltaY;
        }
		...
		public void MouseDown(Point pt, MouseButtons mouseButtons) {
			...
			if(CheckMouse(pt)) {
                pt = ConvertPointToAbsolute(pt);
                ActiveActionsImports.mouse_event(GetMouseFlagsByMouseButtons(mouseButtons, true), pt.X, pt.Y, 0, IntPtr.Zero);
				DoEvents();
			}
		}
		public void MouseUp() {
		...
		public void MouseUp(Point pt, MouseButtons mouseButtons) {
			...
			if(CheckMouse(pt)) {
                pt = ConvertPointToAbsolute(pt);
                ActiveActionsImports.mouse_event(GetMouseFlagsByMouseButtons(mouseButtons, false), pt.X, pt.Y, 0, IntPtr.Zero);
				DoEvents();
			}
		}
		...
		void SendKeyCore(Control control, char key) {
			if( !IsControlExists(control)) return;
