...
Public NotInheritable Class Utils
	...
	Public Shared Function SaveContactPhoto(ByVal newPhoto() As Byte) As String
		...
		Using stream = New MemoryStream(newPhoto)
			...
			Using thumbnail = New ImageThumbnailCreator(original, ImageSizeMode.FillAndCrop).CreateImageThumbnail(New Size(200, 240))
	...
	Public Shared Function NormalizeXmlString(ByVal xmlContent As String) As String
		If String.IsNullOrEmpty(xmlContent) Then
			Return String.Empty
		End If
		Dim xmlString = Regex.Replace(xmlContent, "^.*?(?=<)", "", RegexOptions.IgnorePatternWhitespace)
		Return If(xmlString.StartsWith("<"), xmlString, String.Empty)
	End Function
