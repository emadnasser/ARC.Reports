...
using DevExpress.Web.Demos.ASPxScheduler;
using DevExpress.XtraScheduler;

public partial class Templates_TimeRulerHeaderTemplate : System.Web.UI.Page {
    ...
    void SetTimeRulersProperties() {
        foreach (TimeRuler ruler in ASPxScheduler1.DayView.TimeRulers)
    ...
    protected void ASPxScheduler1_BeforeExecuteCallbackCommand(object sender, SchedulerCallbackCommandEventArgs e) {
        if(e.CommandId == ChangeTimeZoneCallbackCommand.CommandId)
            e.Command = new ChangeTimeZoneCallbackCommand((ASPxScheduler)sender);
    }
}
