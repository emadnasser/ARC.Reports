    ...
    internal class PatientsBuilder {
		...
		private void PopulateBaselineVisit(Visit baselineVisit, bool isMale) {
			...
			if (dice < FormIsFilledChance) {
				...
				demographicFormData.DateOfBirth.Value = dateOfBirth.ToString(CultureInfo.InvariantCulture);
			...
			if (dice < FormIsFilledChance) {
				...
				vitalsFormData.ActualTime.Value = baselineVisit.VisitTime.Value.AddMinutes(randomGenerator.Next(18)*10).ToString(CultureInfo.InvariantCulture);
		...
		public void PopulateDay1Visit(Visit day1Visit, Visit baselineVisit) {
			...
			if (dice < FormIsFilledChance) {
				...
				if (vitalsOriginalForm.FormState == FormState.Completed) {
					...
					vitalsFormData.ActualTime.Value = day1Visit.VisitTime.Value.AddMinutes(randomGenerator.Next(18)*10).ToString(CultureInfo.InvariantCulture);
		...
		private void PopulateDay10Visit(Visit day10Visit, Visit day1Visit) {
			...
			if (dice < FormIsFilledChance) {
				...
				inventoryFormData.ReceiptDate.Value = day1Visit.VisitDate.Value.ToString(CultureInfo.InvariantCulture);
				inventoryFormData.ShipDate.Value = shipDate.ToString(CultureInfo.InvariantCulture);
			...
			if (dice < FormIsFilledChance) {
				...
				if (vitalsOriginalForm.FormState == FormState.Completed) {
					...
					vitalsFormData.ActualTime.Value = day10Visit.VisitTime.Value.AddMinutes(randomGenerator.Next(18)*10).ToString(CultureInfo.InvariantCulture);
		...
		private void FillInvetnoryFormMedicationUsage(double quantityShipped, DateTime shipDate,
			...
			for (int i = 0; i < usagesCount - 1; i++) {
				inventoryFormData.MedicationUsage.Add(new RepeatableInventoryData {
					DateUsed =
						new Question() {Caption = "Date Used", DataType = QuestionDataType.Date, Value = nextUsage.ToString(CultureInfo.InvariantCulture) },
					QuantityUsed =
		...
		public void AddAdverseEventVisit(Patient patient) {
			...
			formData.OnsetDate.Value = onsetDate.Date.ToString(CultureInfo.InvariantCulture);
			formData.OnsetTime.Value = onsetDate.ToString(CultureInfo.InvariantCulture);

			var endDate = onsetDate.AddMinutes(randomGenerator.Next(3, 36)*10);
			formData.EndDate.Value = endDate.Date.ToString(CultureInfo.InvariantCulture);
			formData.EndTime.Value = endDate.ToString(CultureInfo.InvariantCulture);
		}
