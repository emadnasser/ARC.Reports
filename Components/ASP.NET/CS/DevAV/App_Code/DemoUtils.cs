...
public class DemoImageLoader {
    const string
        ...
        EnsureImagesByTimeoutKey = "EnsureImagesByTimeoutKey";
    const int EnsureImagesInterval = 5;
    ...
    string MapPath(string virtualPath) { return Path.Combine(SiteRootPath, virtualPath.Substring(2)); } // ~/

    string siteRootPath;
    string SiteRootPath {
        get {
            if(string.IsNullOrEmpty(siteRootPath))
                siteRootPath = HttpContext.Current.Server.MapPath("~/");
            return siteRootPath;
        }
    }

    public void EnsureImages() {
        if(!Directory.Exists(TempFolderPath) || Directory.GetFiles(TempFolderPath, "*.jpg", SearchOption.AllDirectories).Length == 0)
            LoadImagesFromDB();
        HttpRuntime.Cache.Insert(EnsureImagesByTimeoutKey, this, null, DateTime.UtcNow + TimeSpan.FromMinutes(EnsureImagesInterval),
            Cache.NoSlidingExpiration, CacheItemPriority.NotRemovable, (k, v, r) => { EnsureImages(); });
    }
    ...
    void LoadProductImages() {
        ...
        foreach(var productInfo in productsInfo) {
            if(!Directory.Exists(ProductImagesPath(productInfo.ID)))
                Directory.CreateDirectory(ProductImagesPath(productInfo.ID));
            if(!Directory.Exists(ProductThumbPath(productInfo.ID)))
    ...
    void LoadEmployeeImages() {
        ...
        if(!Directory.Exists(EmployeesPath))
    ...
    void LoadCustomerImages() {
        ...
        if(!Directory.Exists(CrestsPath))
            Directory.CreateDirectory(CrestsPath);
        if(!Directory.Exists(CustomerEmployeesPath))
