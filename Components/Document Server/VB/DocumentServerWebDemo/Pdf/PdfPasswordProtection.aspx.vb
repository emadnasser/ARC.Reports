Imports Microsoft.VisualBasic
Imports System
Imports System.IO
Imports System.Security
Imports System.Drawing
Imports System.Drawing.Imaging
Imports System.Web.Hosting
Imports System.Web.UI
Imports DevExpress.Pdf
Imports DevExpress.Web
Imports DevExpress.Web.Demos

Partial Public Class PdfPasswordProtection
	Inherits Page
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
		Utils.RegisterCssLink(Me, "~/Content/PdfDemos.css")
		ASPxWebControl.SetIECompatibilityModeEdge()
		Using documentProcessor As New PdfDocumentProcessor()
			documentProcessor.LoadDocument(HostingEnvironment.MapPath("~/App_Data/PasswordProtectionDemo.pdf"))
			Using bitmap As Bitmap = documentProcessor.CreateBitmap(1, 900)
				Using stream As New MemoryStream()
					bitmap.Save(stream, ImageFormat.Png)
					pageImage.Src = "data:image/png;base64," & Convert.ToBase64String(stream.GetBuffer())
				End Using
			End Using
		End Using
	End Sub
	Protected Sub OnProtectButtonClick(ByVal source As Object, ByVal e As EventArgs)
		Using documentProcessor As New PdfDocumentProcessor()
			documentProcessor.LoadDocument(HostingEnvironment.MapPath("~/App_Data/PasswordProtectionDemo.pdf"))
			Using stream As Stream = New MemoryStream()
				Dim document As PdfDocument = documentProcessor.Document
				document.Creator = "PDF Document Processor Demo"
				document.Producer = "Developer Express Inc., " & AssemblyInfo.Version
				document.Author = "DevExpress Inc."
				document.Keywords = "PDF, document, server, code, generation, file atatchment"
				document.Subject = "PDF Document Processor - Password Protection"
				document.Title = "PDF Password Protection"
				Dim ownerPassword As New SecureString()
				For Each c As Char In tbOwnerPassword.Text
					ownerPassword.AppendChar(c)
				Next c
				Dim userPassword As New SecureString()
				For Each c As Char In tbUserPassword.Text
					userPassword.AppendChar(c)
				Next c
				Dim encryptionOptions As New PdfEncryptionOptions() With {.OwnerPassword = ownerPassword, .UserPassword = userPassword, .PrintingPermissions = (CType(cbAllowPrinting.Value, PdfDocumentPrintingPermissions)), .DataExtractionPermissions = (CType(cbAllowExtraction.Value, PdfDocumentDataExtractionPermissions)), .ModificationPermissions = (CType(cbAllowModification.Value, PdfDocumentModificationPermissions)), .InteractivityPermissions = (CType(cbAllowInteractivity.Value, PdfDocumentInteractivityPermissions))}
				documentProcessor.SaveDocument(stream, New PdfSaveOptions() With {.EncryptionOptions = encryptionOptions})
				PdfDocumentReponse.SendStream(Response, stream)
			End Using
		End Using
	End Sub
End Class
