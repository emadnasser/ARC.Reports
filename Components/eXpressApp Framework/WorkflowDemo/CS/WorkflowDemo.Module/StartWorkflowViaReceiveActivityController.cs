...
namespace WorkflowDemo.Module {
	...
	public class StartWorkflowViaReceiveActivityController : ViewController {
		private const string WorkflowServerAddress = WorkflowDemoModule.WorkflowServerAddress;
		...
		private void passIntAction_Execute(object sender, ParametrizedActionExecuteEventArgs e) {
			...
			if(definition != null) {
				IPassIntegerData serverWorkflow = ChannelFactory<IPassIntegerData>.CreateChannel(WorkflowDemoModule.WorkflowBinding, new EndpointAddress(WorkflowServerAddress + definition.GetUniqueId()));
		...
		private void passStringAction_Execute(object sender, ParametrizedActionExecuteEventArgs e) {
			IWorkflowDefinition definition = View.ObjectSpace.FindObject<XpoWorkflowDefinition>(CriteriaOperator.Parse("Name = 'Custom start workflow'"));
			if(definition != null) {
				IPassStringData serverWorkflow = ChannelFactory<IPassStringData>.CreateChannel(WorkflowDemoModule.WorkflowBinding, new EndpointAddress(WorkflowServerAddress + definition.GetUniqueId()));
