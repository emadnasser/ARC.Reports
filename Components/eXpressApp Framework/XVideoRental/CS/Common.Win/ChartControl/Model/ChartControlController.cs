...
namespace Common.Win.ChartControl.Model {
    [OnlineRepoLocation("http://goo.gl/V3Fsu")]
        ...
        void ChartControlOnObjectSelected(object sender, HotTrackEventArgs hotTrackEventArgs) {
            ...
            if (!hotTrackEventArgs.Cancel && hotTrackEventArgs.HitInfo.InSeries) {
                ...
                if (argumentDataMember != null) {
                    var collectionSource = Application.CreateCollectionSource(ObjectSpace, View.ObjectTypeInfo.Type,
                                                                              View.Model.Id, CollectionSourceMode.Normal);
                    var argumentValue = ReflectionHelper.Convert(hotTrackEventArgs.HitInfo.SeriesPoint.Argument,
                                                                 argumentDataMember.MemberType);
