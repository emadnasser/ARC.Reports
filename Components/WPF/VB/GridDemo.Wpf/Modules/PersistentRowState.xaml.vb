...
Namespace GridDemo
    <CodeFile("ModuleResources/PersistentRowStateTemplates(.SL).xaml"), CodeFile("ModuleResources/PersistentRowStateClasses.(cs)")>
    Partial Public Class PersistentRowState
        ...
        Private Sub UpdateView()
            If viewListBox.SelectedIndex = 0 Then
                grid.View = CType(FindResource("gridView"), GridViewBase)
            End If
            If viewListBox.SelectedIndex = 1 Then
        ...
        Private Sub SetCursorPositionCore()
            ...
            UnsafeNativeMethods.SetCursorPos(CInt((point.X)), CInt((point.Y)))
        End Sub
        ...
        Public Property SetCursorPositionCommand() As ICommand
            Get
                Return DirectCast(GetValue(SetCursorPositionCommandProperty), ICommand)
            End Get
