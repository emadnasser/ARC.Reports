...
Namespace PropertyGridDemo
    Public Class SelectedElementVisualizer
        ...
        Private Shared Sub PropertyChangedCallback(ByVal d As DependencyObject, ByVal e As DependencyPropertyChangedEventArgs)
            CType(d, SelectedElementVisualizer).SelectedElementChanged(DirectCast(e.OldValue, FrameworkElement), DirectCast(e.NewValue, FrameworkElement))
        End Sub
        Private ReadOnly childrenLocker As New Locker()
        Public Sub New()
        ...
        Protected Overrides Sub OnVisualChildrenChanged(ByVal visualAdded As DependencyObject, ByVal visualRemoved As DependencyObject)
            ...
            childrenLocker.DoLockedActionIfNotLocked(AddressOf ChildrenUpdate)
        End Sub
        ...
        Public Property SelectedElement() As FrameworkElement
            Get
                Return DirectCast(GetValue(SelectedElementProperty), FrameworkElement)
            End Get
        ...
        Protected Overridable Sub SelectedElementChanged(ByVal oldElement As FrameworkElement, ByVal newElement As FrameworkElement)
            If oldElement IsNot Nothing Then
                AddHandler oldElement.LayoutUpdated, AddressOf OnSelectedElementLayoutUpdated
            End If
            If newElement IsNot Nothing Then
                AddHandler newElement.SizeChanged, AddressOf OnSelectedElementLayoutUpdated
            End If
            BuildLayout()
        End Sub
    ...
    Public Class Selector
        ...
        Public Sub New()
            Dim border As New Border() With {.Margin = New Thickness(-2), .BorderBrush = Brushes.Red, .BorderThickness = New Thickness(2), .Opacity = 0.35, .HorizontalAlignment = HorizontalAlignment.Stretch, .VerticalAlignment = VerticalAlignment.Stretch}
            Children.Add(border)
        End Sub
    End Class
End Namespace
