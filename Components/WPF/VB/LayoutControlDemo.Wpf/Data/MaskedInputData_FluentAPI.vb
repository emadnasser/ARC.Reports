...
Namespace DevExpress.Xpf.LayoutControlDemo
    ...
    Public NotInheritable Class MaskedInputMetadata

        Private Sub New()
        End Sub

        Public Shared Sub BuildMetadata(ByVal builder As MetadataBuilder(Of MaskedInputData_FluentAPI))
'            #Region "Numeric masks"
            builder.Property(Function(x) x.CurrencyProperty).DisplayName("Currency (Culture-specific)").NumericMask(PredefinedMasks.Numeric.Currency)
            builder.Property(Function(x) x.EuroCurrencyProperty).DisplayName("Currency (EURO)").NumericMask(PredefinedMasks.Numeric.Currency).MaskCulture(New CultureInfo("fr-FR"))
            builder.Property(Function(x) x.PercentProperty).DisplayName("Percents").NumericMask(PredefinedMasks.Numeric.Percent)
            builder.Property(Function(x) x.PercentMultBy100Property).DisplayName("Percents (multiplied by 100)").NumericMask(PredefinedMasks.Numeric.PercentFractional)
...
'            #Region "DateTime masks"
            ...
            builder.Property(Function(x) x.GeneralDateShortTimeProperty).DisplayName("General date, short time").DateTimeMask(PredefinedMasks.DateTime.GeneralDateShortTime)
            builder.Property(Function(x) x.GeneralDateLongTimeProperty).DisplayName("General date, long time").DateTimeMask(PredefinedMasks.DateTime.GeneralDateLongTime)
            ...
            builder.Property(Function(x) x.UniversalSortableDateTimeProperty).DisplayName("Universal sortable date time").DateTimeMask(PredefinedMasks.DateTime.UniversalSortableDateTime)
...
'            #Region "Layout"
      builder.DataFormLayout().TabbedGroup("Tabs").Group("Numeric masks").GroupBox("Standard").ContainsProperty(Function(x) x.CurrencyProperty).ContainsProperty(Function(x) x.EuroCurrencyProperty).ContainsProperty(Function(x) x.PercentProperty).ContainsProperty(Function(x) x.PercentMultBy100Property).ContainsProperty(Function(x) x.FixedPointProperty).ContainsProperty(Function(x) x.NumberProperty).ContainsProperty(Function(x) x.Decimal4DigitsProperty).EndGroup().GroupBox("Custom").ContainsProperty(Function(x) x.CustomNumericMaskPropery1).ContainsProperty(Function(x) x.CustomNumericMaskPropery2).EndGroup().EndGroup().Group("DateTime masks").GroupBox("Standard").ContainsProperty(Function(x) x.ShortDateProperty).ContainsProperty(Function(x) x.LongDateProperty).ContainsProperty(Function(x) x.ShortTimeProperty).ContainsProperty(Function(x) x.LongTimeProperty).ContainsProperty(Function(x) x.FullDateShortTimeProperty).ContainsProperty(Function(x) x.FullDateLongTimeProperty).ContainsProperty(Function(x) x.GeneralDateShortTimeProperty).ContainsProperty(Function(x) x.GeneralDateLongTimeProperty).ContainsProperty(Function(x) x.MonthDayProperty).ContainsProperty(Function(x) x.YearMonthProperty).ContainsProperty(Function(x) x.SortableDateTimeProperty).ContainsProperty(Function(x) x.UniversalSortableDateTimeProperty).EndGroup().GroupBox("Custom").ContainsProperty(Function(x) x.CustomDateTimeMaskPropery1).ContainsProperty(Function(x) x.CustomDateTimeMaskPropery2).EndGroup().EndGroup().Group("RegEx masks").ContainsProperty(Function(x) x.PhoneProperty).ContainsProperty(Function(x) x.SocialSequrityProperty).ContainsProperty(Function(x) x.LongZipCodeProperty).ContainsProperty(Function(x) x.ShortZipCodeProperty).EndGroup()
