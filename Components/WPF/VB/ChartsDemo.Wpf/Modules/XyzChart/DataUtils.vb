...
Namespace ChartsDemo
    Public NotInheritable Class Chart3DUtils
        ...
        Public Shared Function CreateFunctionItemsData() As ObservableCollection(Of Function3DItemData)
            Dim data As New ObservableCollection(Of Function3DItemData)()
            data.Add(New Function3DItemData() With {.Type = FunctionsType.First, .Image = CreateFunctionImage(1)})
            data.Add(New Function3DItemData() With {.Type = FunctionsType.Second, .Image = CreateFunctionImage(2)})
            data.Add(New Function3DItemData() With {.Type = FunctionsType.Third, .Image = CreateFunctionImage(3)})
            data.Add(New Function3DItemData() With {.Type = FunctionsType.Four, .Image = CreateFunctionImage(4)})
            data.Add(New Function3DItemData() With {.Type = FunctionsType.Five, .Image = CreateFunctionImage(5)})
            Return data
        End Function
        Private Shared Function CreateFunctionImage(ByVal index As Integer) As BitmapImage
        ...
        Public Shared Function ReadStarsData() As ObservableCollection(Of StarData)
            Dim fileName As String = "starsdata.csv"
            Dim dataSource As New ObservableCollection(Of StarData)()
            Try
                ...
                Using reader As New StreamReader(info.Stream)
                    Do While Not reader.EndOfStream
                        ...
                        Dim data As New StarData()
                        data.HipID = Integer.Parse(values(0), CultureInfo.InvariantCulture)
    ...
    Public Class DataPointHelper
        Public Shared Function GenerateRandomData(ByVal pointsCount As Integer, Optional ByVal maxValue As Integer = 200) As ObservableCollection(Of Point3D)
            Dim rand As New Random()
            Dim data As New ObservableCollection(Of Point3D)()
