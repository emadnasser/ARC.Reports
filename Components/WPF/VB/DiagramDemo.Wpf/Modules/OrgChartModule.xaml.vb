...
Namespace DiagramDemo
    <CodeFile("ViewModels/OrgChartViewModel.cs"), CodeFile("Resources/OrgChartModuleResources.xaml")>
    Partial Public Class OrgChartModule
        ...
        Private Function CreateViewModel() As OrgChartViewModel
            ...
            Dim viewModel_Renamed = ViewModelSource.Create(Function() New OrgChartViewModel(EmployeesData.Employees.ToArray(), GetOrgChartTemplates()))
            AddHandler viewModel_Renamed.SelectedTemplateChanged, Sub(_d, _e) orgChartBehavior.Refresh()
        ...
        Private Function GetOrgChartTemplates() As EmployeeTemplate()
            ...
            Dim templateImages = templateNames.Select(Function(x) String.Format("images/orgchart/{0}.png", x.Replace(" ", String.Empty).ToLower())).Select(Function(x) DiagramDemoFileHelper.GetResourceStream(x))
        ...
        Private Sub GenerateOrgChartItem(ByVal sender As Object, ByVal e As DiagramGenerateItemEventArgs)
            If ViewModel IsNot Nothing AndAlso ViewModel.SelectedTemplate IsNot Nothing Then
