    ...
    Public Class InlineImage
        ...
        Public Property InlineImageViewModel() As InlineImageViewModel
            ...
            Protected Set(ByVal value As InlineImageViewModel)
                ...
                AddHandler DirectCast(inlineImageViewModelCore, INotifyPropertyChanged).PropertyChanged, AddressOf OnInlineImageViewModelPropertyChanged
                UpdateImage()
            End Set
        End Property
        ...
        End Sub
        Private Function GetPath(ByVal size As Size) As System.Windows.Shapes.Path
            Dim image As New BitmapImage(New Uri(InlineImageViewModel.ImageSource, UriKind.RelativeOrAbsolute))
            image.BaseUri = BaseUriHelper.GetBaseUri(Me)
            Dim brush As New ImageBrush() With {.ImageSource = image}
            ...
            path.SetBinding(System.Windows.Shapes.Path.DataProperty, New Binding("PathData") With {.Source = Me, .Mode = BindingMode.OneWay})
            Return path
        End Function
        Protected Overrides Sub OnRender(ByVal dc As DrawingContext)
            MyBase.OnRender(dc)
            Dim dv As New DrawingVisual()
            Dim vb As New VisualBrush(GetPath(RenderSize))
            dc.DrawRectangle(vb, Nothing, New Rect(New Point(), RenderSize))
        End Sub
        Private Sub UpdateImage()
            Dim size As New Size(DefaultSize.Width * InlineImageViewModel.Scale, DefaultSize.Height * InlineImageViewModel.Scale)
            Width = size.Width
            Height = size.Height
            InvalidateVisual()
        End Sub
