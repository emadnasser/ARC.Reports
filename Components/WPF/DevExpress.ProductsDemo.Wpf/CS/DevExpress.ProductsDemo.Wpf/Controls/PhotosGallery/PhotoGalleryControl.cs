...
namespace ProductsDemo {
    ...
    public class PhotoGalleryPanel : Panel {
        ...
        double GetProgress(int elementNumber, int elementCount) {
            if (Gallery == null)
                return 1.0;
            double step = 1.0 / (elementCount + 1);
            double result = 0.5 * (Gallery.AnimationProgress - elementNumber * step) / step;
            result = Math.Max(0.0, Math.Min(1.0, result));
            return double.IsNaN(result) ? 0 : result;
        }
        void CalculateLayout(Size constraint, int elementCount) {
            double sizeRatio = (constraint.Width > 0 && constraint.Height > 0) ? constraint.Width / constraint.Height * 0.5 : 1;
            rowCount = (int)Math.Max(1, Math.Round(Math.Sqrt(elementCount / sizeRatio)));
            columnCount = (int)Math.Max(1, Math.Ceiling((double)elementCount / (double)rowCount));
        }
        ...
        protected override Size ArrangeOverride(Size arrangeSize) {
            for (int i = 0; i < Children.Count; i++) {
                ...
                Rect layout = CalculateElementLayout(Children[i].DesiredSize, i);
                Rect animatedRect = AnimateRect(layout, i, Children.Count);
