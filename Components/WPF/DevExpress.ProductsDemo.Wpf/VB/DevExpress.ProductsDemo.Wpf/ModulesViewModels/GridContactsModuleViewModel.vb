    ...
    Public Class GridContactsModuleViewModel
        ...
        Private Sub InitializeIndexBarSource()
            ...
            For Each contact As Contact In ItemsSource
                ...
                If source.Contains(firstChar) Then
                    Continue For
                End If
        ...
        Private Sub UpdateFilteredItemsSource()
            Dim source As New ObservableCollection(Of Contact)()
            For Each contact As Contact In ItemsSource
                If SelectedIndexBarString = IndexBarSource(0) OrElse ((Not String.IsNullOrEmpty(contact.LastName)) AndAlso contact.LastName.ToUpper().StartsWith(SelectedIndexBarString)) Then
                    source.Add(contact)
                End If
            Next contact
            FilteredItemsSource = source
        End Sub
        Protected Overrides Sub OnItemsSourceCollectionChanged(ByVal sender As Object, ByVal e As System.Collections.Specialized.NotifyCollectionChangedEventArgs)
        ...
        Protected Overrides Function GetColumns() As List(Of GridColumnModel)
            ...
            Dim columns_Renamed As New List(Of GridColumnModel)()
            columns_Renamed.Add(New GridColumnModel() With {.Name = "Gender", .DisplayName = String.Empty, .AllowEditing = DefaultBoolean.False, .Width = 40, .FixedWidth = True})
            columns_Renamed.Add(New GridColumnModel() With {.Name = "Name.FullName", .DisplayName = "Name", .AllowEditing = DefaultBoolean.False, .GroupInterval = ColumnGroupInterval.Alphabetical})
            columns_Renamed.Add(New GridColumnModel() With {.Name = "Email", .AllowEditing = DefaultBoolean.False, .Width = 182, .FixedWidth = True})
            columns_Renamed.Add(New GridColumnModel() With {.Name = "Address.State", .DisplayName = "State", .AllowEditing = DefaultBoolean.False, .Width = 46, .FixedWidth = True})
            columns_Renamed.Add(New GridColumnModel() With {.Name = "Address.City", .DisplayName = "City", .AllowEditing = DefaultBoolean.False, .Width = 80, .FixedWidth = True})
            columns_Renamed.Add(New GridColumnModel() With {.Name = "Phone", .AllowEditing = DefaultBoolean.False, .Mask = "(\(\d\d\d\)) \d{1,3}-\d\d\d\d", .Width = 90, .FixedWidth = True})
            columns_Renamed.Add(New GridColumnModel() With {.Name = "Activity", .AllowEditing = DefaultBoolean.False, .Width = 70, .FixedWidth = True})
        ...
        Public Sub ShowEditContactWindow()
            ...
            End If
            isNew = False
            ShowEditContactWindowCore(DirectCast(SelectedItem.Clone(), Contact))
        End Sub
        ...
        Private Function GetCities() As List(Of String)
            ...
            Dim cities_Renamed As System.Collections.IEnumerable = (
                From contact In ItemsSource
                Select contact.Address.City).OrderBy(Function(s) s).Distinct()
            Return cities_Renamed.Cast(Of String)().ToList()
        End Function
        Private Function GetStates() As List(Of String)

            Dim states_Renamed As System.Collections.IEnumerable = (
                From contact In ItemsSource
                Select contact.Address.State).OrderBy(Function(s) s).Distinct()
            Return states_Renamed.Cast(Of String)().ToList()
        End Function
    End Class
