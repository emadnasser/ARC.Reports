...
Namespace DevExpress.Demos.SalesDBGenerator
    Public Class SalesGenerator
        ...
        Private minRequiredDate As New Date(2015, 1, 1)
        ...
        Private Sub Generate(ByVal connection As OleDbConnection, ByVal startDate As Date, ByVal daysCount As Integer)
            Using command As New OleDbCommand()
                ...
                For n As Integer = 0 To daysCount - 1
                    Console.Write("{0} of {1}" & ControlChars.Cr, n + 1, daysCount)
        ...
        End Function
        #Region "Progress"
        Public Event GenerationStart As SalesDemo.Model.ProgressEventHandler Implements DevExpress.SalesDemo.Model.IDataGenerator.GenerationStart
        Public Event GenerationProgress As SalesDemo.Model.ProgressEventHandler Implements DevExpress.SalesDemo.Model.IDataGenerator.GenerationProgress
        Public Event GenerationComplete As SalesDemo.Model.ProgressEventHandler Implements DevExpress.SalesDemo.Model.IDataGenerator.GenerationComplete
        ...
        Private products() As Product = {
            New Product() With {.Id = 1, .Name = "Eco Max", .Price = 2500},
            New Product() With {.Id = 2, .Name = "Eco Supreme", .Price = 2000},
            New Product() With {.Id = 3, .Name = "EnviroCare", .Price = 1750},
            New Product() With {.Id = 4, .Name = "EnviroCare Max", .Price = 2800},
            New Product() With {.Id = 5, .Name = "SolarOne", .Price = 1500},
            New Product() With {.Id = 6, .Name = "SolarMax", .Price = 2250}
        }
        Private regions() As Region = {
            New Region() With {.Id = 1, .Name = "North America"},
            New Region() With {.Id = 2, .Name = "South America"},
            New Region() With {.Id = 3, .Name = "Europe"},
            New Region() With {.Id = 4, .Name = "Asia"},
            New Region() With {.Id = 5, .Name = "Australia"}
        }
        Private channels() As Channel = {
            New Channel() With {.Id = 1, .Name = "Direct"},
            New Channel() With {.Id = 2, .Name = "VARs"},
            New Channel() With {.Id = 3, .Name = "Consultants"},
            New Channel() With {.Id = 4, .Name = "Resellers"},
            New Channel() With {.Id = 5, .Name = "Retail"}
        }
        Private sectors() As Sector = {
            New Sector() With {.Id = 1, .Name = "Energy"},
            New Sector() With {.Id = 2, .Name = "Manufacturing"},
            New Sector() With {.Id = 3, .Name = "Telecom"},
            New Sector() With {.Id = 4, .Name = "Insurance"},
            New Sector() With {.Id = 5, .Name = "Banking"},
            New Sector() With {.Id = 6, .Name = "Health"}
        }
