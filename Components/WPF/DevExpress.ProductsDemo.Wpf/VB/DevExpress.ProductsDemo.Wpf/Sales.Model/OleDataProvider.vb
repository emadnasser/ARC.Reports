...
Namespace DevExpress.SalesDemo.Model
    Public Class OleDataProvider
        ...
        Protected Function BuildQueryBySales(ByVal start As Date, ByVal [end] As Date, ByVal period As GroupingPeriod) As String
            ...
            End If
            Dim query As String = String.Format("SELECT" & ControlChars.CrLf & _
"                                                'TOTAL' AS Name," & ControlChars.CrLf & _
"	                                            SUM(Total_Cost) AS TotalCost, " & ControlChars.CrLf & _
"	                                            SUM(Units) AS Units" & ControlChars.CrLf & _
"                                                {0}" & ControlChars.CrLf & _
"                                           FROM " & ControlChars.CrLf & _
"                                                Sales" & ControlChars.CrLf & _
"                                           WHERE " & ControlChars.CrLf & _
"                                                Sale_Date BETWEEN @PeriodStart and @PeriodEnd" & ControlChars.CrLf & _
"                                           ", groupByExpression)
            If Not String.IsNullOrEmpty(groupBy) Then
        ...
        Protected Function BuildJoinQuery(ByVal start As Date, ByVal [end] As Date, ByVal period As GroupingPeriod, ByVal column As GroupColumn) As String
            ...
            If column <> GroupColumn.None Then
                Dim name As String = System.Enum.GetName(GetType(GroupColumn), column)
                keyColumn = name & "Id"
                tableName = name & "s"
                columnName = name & "_Name"
            End If
            Dim query As String = String.Format("SELECT" & ControlChars.CrLf & _
"	                                        {1} as Name," & ControlChars.CrLf & _
"	                                        SUM(Sales.Total_Cost) as TotalCost, " & ControlChars.CrLf & _
"	                                        SUM(Sales.Units) as Units" & ControlChars.CrLf & _
"                                            {2}" & ControlChars.CrLf & _
"                                        FROM " & ControlChars.CrLf & _
"                                            Sales" & ControlChars.CrLf & _
"                                        INNER JOIN " & ControlChars.CrLf & _
"                                            {3} as LinkedTable on Sales.{0} = LinkedTable.Id" & ControlChars.CrLf & _
"                                        WHERE " & ControlChars.CrLf & _
"                                            Sales.Sale_Date BETWEEN @PeriodStart and @PeriodEnd  " & ControlChars.CrLf & _
"                                        GROUP BY" & ControlChars.CrLf & _
"                                            {1} {4}", keyColumn, [Cstr]("LinkedTable." & columnName), groupByField, tableName, groupByCondition)
        ...
        Private Shared Function GetGroupBy(ByVal period As GroupingPeriod, ByVal column As String) As String
            ...
            Return String.Empty
        End Function
        Private Shared MinDate As New Date(1900, 1, 1, 0, 0, 0)
        Private Shared MaxDate As New Date(2100, 12, 31, 11, 59, 59)
