    ...
    Public Class PhotoGalleryPanel
        ...
        Private Function GetProgress(ByVal elementNumber As Integer, ByVal elementCount As Integer) As Double
            If Gallery Is Nothing Then
                Return 1.0
            End If
            Dim [step] As Double = 1.0 \ (elementCount + 1)
            Dim result As Double = 0.5 * (Gallery.AnimationProgress - elementNumber * [step]) / [step]
        ...
        Private Sub CalculateLayout(ByVal constraint As Size, ByVal elementCount As Integer)
            Dim sizeRatio As Double = If(constraint.Width > 0 AndAlso constraint.Height > 0, constraint.Width / constraint.Height * 0.5, 1)
            rowCount = CInt(Math.Max(1, Math.Round(Math.Sqrt(elementCount / sizeRatio))))
            columnCount = CInt(Math.Max(1, Math.Ceiling(CDbl(elementCount) / CDbl(rowCount))))
        End Sub
        ...
        Protected Overrides Function MeasureOverride(ByVal availableSize As Size) As Size
            ...
            Dim elementSize As New Size(constraint.Width \ columnCount, constraint.Height \ rowCount)
        ...
        Protected Overrides Function ArrangeOverride(ByVal arrangeSize As Size) As Size
            For i As Integer = 0 To Children.Count - 1
                ...
                Dim layout As Rect = CalculateElementLayout(Children(i).DesiredSize, i)
                Dim animatedRect As Rect = AnimateRect(layout, i, Children.Count)
                Children(i).Arrange(animatedRect)
            Next i
            Return arrangeSize
        End Function
    End Class
