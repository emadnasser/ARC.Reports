...
Namespace DevExpress.SalesDemo.Wpf.ViewModel
    Public Class PerformanceBarChartViewModel
        ...
        Public Shared Shadows Function Create(ByVal moduleType As ModuleType, ByVal mode As PerformanceViewMode) As PerformanceBarChartViewModel
            Return ViewModelSource.Create(Function() New PerformanceBarChartViewModel(moduleType, mode))
        End Function
        Protected Sub New()
            Me.New(ModuleType.Products, PerformanceViewMode.Daily)
        End Sub
        Protected Sub New(ByVal moduleType As ModuleType, ByVal mode As PerformanceViewMode)
            MyBase.New(moduleType, mode)
        End Sub
        Protected Overrides Sub ValidateMode(ByVal moduleType As ModuleType, ByVal mode As PerformanceViewMode)
            If moduleType = ViewModel.ModuleType.Products OrElse moduleType = ViewModel.ModuleType.Sectors OrElse moduleType = ViewModel.ModuleType.Regions Then
                Return
            End If
        ...
        Protected Overrides Sub UpdateVolumeLables()
            Dim requestDataCore As Action(Of String, DateTimeRange, Action(Of SalesGroup)) = Sub(id, range, callback) RequestData(id, Function(x) x.GetTotalSalesByRange(range), callback)
