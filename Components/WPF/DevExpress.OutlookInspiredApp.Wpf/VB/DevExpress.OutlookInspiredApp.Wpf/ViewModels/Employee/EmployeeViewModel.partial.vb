    ...
    Partial Public Class EmployeeViewModel
        ...
        Protected Overrides Sub OnEntityChanged()
            ...
            If Entity IsNot Nothing Then
                Xpf.DemoBase.Helpers.Logger.Log(String.Format("OutlookInspiredApp: Edit Employee: {0}",If(String.IsNullOrEmpty(Entity.FullName), "<New>", Entity.FullName)))
            End If
        End Sub
        Protected Overrides Function TryClose() As Boolean
            Dim closed = MyBase.TryClose()
            If closed Then
                DocumentManagerService.Documents.First(Function(x) x.Content Is Me).DestroyOnClose = True
            End If
            Return closed
        End Function
